...
seperate_digits = []
files_of_handwriting = ['louietrainingimages/louiedigits2.jpg']
...
for digit in seperate_digits:
    ...
    title = 'Digit of image for handwriting, predicted as ' + str(prediction)
    show_digit(processed_digit, title)

    correct_digit = input('Predicted as digit ' + str(prediction))
    # correct_digit = None

    if not correct_digit:
        matrices_for_digits[prediction].append(processed_digit)
    elif correct_digit == ' ':
        continue
    else:
        correct_digit = int(correct_digit)
        matrices_for_digits[correct_digit].append(processed_digit)
...
store_matrices_to_file(training_matrices_for_handwriting, 'louietrainingimages/page2digitmatrices.txt')
