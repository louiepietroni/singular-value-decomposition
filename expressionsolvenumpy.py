...
print('Residual matrices loaded from file')

matrices_for_residual_numpy = [np.array(residual_matrix.data) for residual_matrix in matrices_for_residual]
print('Residual matrices converted to numpy')


def predict_digit(digit_numpy):
    residuals = []
    for residual_matrix_numpy in matrices_for_residual_numpy:
        residual = calculate_residual(digit_numpy, residual_matrix_numpy)
...
def calculate_residual(digit_numpy, residual_numpy):
    # print(residual_numpy.shape, digit_numpy.shape, 'shape of res then dig')

    residual_by_digit = residual_numpy @ digit_numpy
    b = residual_by_digit.transpose()
    # residual_t = np.transpose(residual_by_digit)
    # a = residual_t @ residual_by_digit
    a = b @ residual_by_digit

    # residual_by_digit_for_eigen_value = Matrix.multiply_left_by_transpose(residual_by_digit)
    # print(a)
    a_eig_vals = np.linalg.eigvals(a)
    largest = max(a_eig_vals)
    singular_value = largest ** (1/2)
...
def show_digit(matrix_numpy, title=''):
    image = plt.imshow(matrix_numpy)
    image.set_cmap('gray')
    plt.axis('off')
    plt.title(title)
    plt.show()
...
def predict_digit_matrix(matrix_numpy):
    flat_matrix_numpy = matrix_numpy.flatten()
    arr = flat_matrix_numpy[:, np.newaxis]

    prediction = predict_digit(arr)
    return prediction
...
print('Got seperate digits')

seperate_digits = [scale_processed_digit_to_size(digit) for digit in seperate_digits]
print('scaled and processed')
...
print('Starting predicting digits now with numpy library')

for index, digit_numpy in enumerate(seperate_digits_numpy):
    # title = 'Seperated digit of matrix, ' + str(index)
    # show_digit(digit, title)

    prediction = predict_digit_matrix(digit_numpy)
